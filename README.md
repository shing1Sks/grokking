...
├── grokking_logs.json  # Raw training logs — 1,500 steps × 5 metrics
├── README.md           # This file
```

> `grokking_slice.json` and `grokking_slice_str.txt` are build artifacts — safe to delete.
...
The only network requests are Google Fonts and Chart.js from cdnjs — both are cosmetic. The page works without them (fonts fall back to system fonts, chart requires Chart.js).
...
| **Auto-play** | Play/pause button with 0.5×–4× speed control. Loops the full training run as a ~18-second animation. |
...
`grokking_logs.json` contains 1,500 logged checkpoints (steps 100–150,000) with five fields:
...
The visualization uses only the first 200 checkpoints (steps 100–20,000), embedded directly in the HTML. Everything interesting happens before step 20,000 — beyond that, both metrics are flat at 100%.
...
- **Vertical annotation lines** are drawn via a custom Chart.js plugin registered inline — no `chartjs-plugin-annotation` dependency.
- **Training set simulation** uses a Mulberry32 seeded PRNG + Fisher-Yates shuffle (seed 42, 30% split) to reproduce which pairs the model saw, enabling the step-1000 inference demo.
